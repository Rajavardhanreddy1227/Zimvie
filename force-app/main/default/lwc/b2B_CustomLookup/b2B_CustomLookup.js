...
import fetchRecords from '@salesforce/apex/B2B_ReusableLookupController.fetchRecords';
/** The delay used when debouncing event handlers before invoking Apex. */
const DELAY = 500;

export default class B2B_CustomLookup extends LightningElement {
    ...
    get getVariant(){
        return this.label?.length > 0 ? '' : 'label-hidden';
    }
    ...
    fetchSobjectRecords(loadEvent) {
        fetchRecords({
            ...
            if (loadEvent && result) {
                ...
                console.log('Custom Look Up',this.recordsList);
            } else {
                this.recordsList = [];
            }
        }).catch(error => {
    ...
    handleCommit() {
        console.log("handleCommit handleRemoveFlag", this.handleRemoveFlag);
       if(!this.handleRemoveFlag){
    ...
    handleSelect(event) {
        ...
        const selectedEvent = new CustomEvent('valueselected', {
            detail: {index : this.index,value:selectedRecord}
        });

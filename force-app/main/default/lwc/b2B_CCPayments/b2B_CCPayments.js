...
import { getCountryPicklist } from './countryPicklistJSON';
...
export default class B2B_CCPayments extends LightningElement {
     ...
     _walletToEdit;
     _isConnected = false;
     countryOptions = getCountryPicklist();
    ...
    @api
    get walletToEdit(){
        return this._walletToEdit;
    }

    set walletToEdit(value){
        if(value){
            this._cardHolderName = value.CardHolderName__c;
            this._cardType = value.Card_Type__c;
            this._cardNumber = value.Display_Card_Number__c;
            this._expiryMonth = value.ExpiryMonth__c.toString();
            this._expiryYear = value.ExpiryYear__c.toString();
        }
    }
    ...
    handleStreet2Change(event){
        this._street2 = event.target.value;
        event.target.reportValidity();
    }

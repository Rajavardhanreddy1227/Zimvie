...
export default class OrthoChartTable extends OrthoChart {
  @api chartSource;
  @api reportLink;
  ...
  get data(){
    return this.hasChartData && this.chartData.datasets.flatMap((dataset, idx) => {
      return {label: dataset.label, cysales: dataset.data[0], lysales: dataset.data[1], ads: (dataset.data[0]/dataset.data[1]).toFixed(2)};
    });
  }
